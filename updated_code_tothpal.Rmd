...
knitr::opts_knit$set(root.dir = 'C:/Users/dmw63/Desktop/My documents h/LAB/growth curve analyses/')
...
getwd()
mlst1<-read.csv('./GrowthVariation/pcat_mlst_results.csv')
...
civ1<-read.csv('C:/Users/dmw63/Desktop/My documents h/color palettes/cividis rgb.csv')
...
heatmap(as.matrix(coef.st.regs.plot[,c("coef.st.lag.length.1.2","coef.st.fixed.od.1.2","coef.st.max.od.1.2","coef.growth.rate.1.2")]), scale='column', col=civ1.pal2)
...
```{r st.oxygen.interact, fig.width=10.6, fig.height=5.3}
sub<-d1d
...
plot(cis.mod4.int$st.num,cis.mod4.int[,'est.'], bty='l', ylim=c(-0.02,0.18), pch=16, xaxt='n',xlab='',
...
carr1<-cov1[,c('st','SLEEMANCARRN','TROTTERCARRN','NORWAYCARR_PRE','MASSCARR01' ,'GREECECARRN')]
...
carr.comp1a[carr.comp$st=='14', c('coef.st.fixed.od.1.2','coef.st.max.od.1.2', 'coef.st.lag.length.1.2',
...
carr.comp[carr.comp$st=='14', c('coef.st.fixed.od.1.2','coef.st.max.od.1.2', 'coef.st.lag.length.1.2',
                          'coef.growth.rate.1.2')]<-0 #reference level
carr.comp$sqrt.ave.carr.pre<- sqrt(carr.comp$ave_carr_pre)
...
carr.vars<-c('SLEEMANCARRN','TROTTERCARRN','NORWAYCARR_PRE','MASSCARR01' ,'GREECECARRN')
for(i in carr.vars){
...
print(cbind.data.frame(covar.list,aic))
  plot(carr.comp1a[,covar.list[3]], carr.comp1a[,i], bty='l')

...
for(i in 1:10){
  decline.quant1<-round((mean(derivs.all[i,][derivs.all[i,]<0])),2)
  plot(derivs.all[i,], type='l', bty='l', main=decline.quant1, ylim=c(-0.4, 1))
  abline(h=0)
}

#Quantify intensity of the decline after peak
decline.quant<- apply(derivs.all,1, function(x) mean(x[x<0], na.rm=T))

test1<-cbind.data.frame(decline.quant,d1b[,1:10])
test1.tigr<-test1[test1$Diagnosis==8,]
...
d1c<-cbind.data.frame(d1b[,c('max.od','ID','st','anaerobic','temp', 'Diagnosis')],max.deriv, decline.quant, "max.deriv2.time"=max.deriv2.time[,c('max.deriv2.time')])
d1d<-d1c[d1c$anaerobic %in% c('1','2') & d1c$Diagnosis %in% c('0', '1','2','3','4','5','6'),]
d1d$anaerobic<-factor(d1d$anaerobic)
  d1d$temp<-factor(d1d$temp)
  d1d$st<-factor(d1d$st)
...
#  require(devtools)
#install_version("nlme", version = "3.1-137", repos = "http://cran.us.r-project.org")
 # library(nlme)

  mod3<- lme(max.od  ~ st + temp*anaerobic,  random = ~ 1|ID,data=d1d)
  #summary(mod3)
...
#Max OD
d1d2<-d1d[d1d$Diagnosis %in% c('5','0'),]
mod.st<-lme(max.od  ~ st +  anaerobic*temp + Diagnosis*anaerobic ,  random = ~ 1|ID,data=d1d2)
...
mod.st<-lme(max.od  ~ st +  temp + Diagnosis ,  random = ~ 1|ID,data=d1d2[d1d2$anaerobic=='1',])
coef.fe.mod.st<-fixef(mod.st)
coef.st1<-coef.fe.mod.st[substr(names(coef.fe.mod.st),1,2)=='st' ]

mod.st<-lme(max.od  ~ st +  temp + Diagnosis ,  random = ~ 1|ID,data=d1d2[d1d2$anaerobic=='2',])
coef.fe.mod.st<-fixef(mod.st)
coef.st2<-coef.fe.mod.st[substr(names(coef.fe.mod.st),1,2)=='st' ]

#Time to start of lag growth (max deriv 2)
lag.length.mod<-lme(max.deriv2.time  ~ st +anaerobic*temp+anaerobic*Diagnosis  ,  random = ~ 1|ID, data=d1d2)
...
mod.st.fixed.od<-lme(od.fixed.time  ~ st +  anaerobic*temp + anaerobic*Diagnosis ,  random = ~ 1|ID,
                     data=od.fixed.time[od.fixed.time$Diagnosis %in% c('5','0'),])
...
mod.st.fixed.od<-lme(od.fixed.time  ~ st +  temp + Diagnosis ,  random = ~ 1|ID,
                     data=od.fixed.time[od.fixed.time$anaerobic==1 & od.fixed.time$Diagnosis %in% c('5','0'),])
coef.fe.mod.st.fixed.od<-fixef(mod.st.fixed.od)
...
mod.st.fixed.od2<-lme(od.fixed.time  ~ st +  temp + Diagnosis ,  random = ~ 1|ID,
                     data=od.fixed.time[od.fixed.time$anaerobic==2 & od.fixed.time$Diagnosis %in% c('5','0'),])
...
## Evaluate the difference in rank order by serotype in aerobic/anaerobic
```{r, fig.width=4, fig.height=4}
coef.st.fixed.od.1
coef.st.fixed.od.2
plot.range<-range(c(coef.st.fixed.od.1,coef.st.fixed.od.2))
plot(coef.st.fixed.od.2,coef.st.fixed.od.1, ylim=plot.range, xlim=plot.range)
abline(a=0, b=1)

#all.perm<-expand.grid(coef.st.fixed.od.1,coef.st.fixed.od.2)
comb1<-expand.grid(coef.st.fixed.od.1,coef.st.fixed.od.1)
comb2<-expand.grid(coef.st.fixed.od.2,coef.st.fixed.od.2)

#Evaluate difference in OD between serotypes and compare anaerobic(1) and aerobic(2)
diff1<- comb1[,1]-comb1[,2]
diff2<- comb2[,1]-comb2[,2]
plot(diff1, diff2)
abline(a=0, b=1, h=0.01, v=0.01)

#Flag pairs where direction flips (where strain 1 is more fit in 1 conditions and less fit in the other)
flag.pair<- rep(0, times=length(diff1))
flag.pair[diff1>0.005 & diff2< -0.005 ]<-1
flag.sts<-cbind( names(comb1$Var1)[flag.pair==1], names(comb1$Var2)[flag.pair==1]  )
flag.sts<-flag.sts[order(flag.sts[,1], flag.sts[,2]),]
#names(comb1$Var1)==names(comb2$Var1) ##check

```

Same, but for IPD only
```{r}
od.fixed.cdc<-od.fixed.time[substr(od.fixed.time$ID,1,3)=='CDC' & od.fixed.time$temp %in% c('33','35'),]
mod.st.fixed.od2.cdc<-lm(od.fixed.time  ~ st +  temp  ,
                     data=od.fixed.cdc[od.fixed.cdc$anaerobic==2,])
coef.fe.mod.st.fixed.od2.cdc<-coef(mod.st.fixed.od2.cdc)
mod.st.fixed.od1.cdc<-lm(od.fixed.time  ~ st +  temp  ,
                     data=od.fixed.cdc[od.fixed.cdc$anaerobic==1,])
coef.fe.mod.st.fixed.od1.cdc<-coef(mod.st.fixed.od1.cdc)

##
plot.range<-range(c(coef.fe.mod.st.fixed.od1.cdc,coef.fe.mod.st.fixed.od2.cdc))
plot(coef.fe.mod.st.fixed.od2.cdc,coef.fe.mod.st.fixed.od1.cdc, ylim=plot.range, xlim=plot.range)
abline(a=0, b=1)

#all.perm<-expand.grid(coef.st.fixed.od.1,coef.st.fixed.od.2)
comb1<-expand.grid(coef.fe.mod.st.fixed.od1.cdc,coef.fe.mod.st.fixed.od1.cdc)
comb2<-expand.grid(coef.fe.mod.st.fixed.od2.cdc,coef.fe.mod.st.fixed.od2.cdc)

#Evaluate difference in OD between serotypes and compare anaerobic(1) and aerobic(2)
diff1<- comb1[,1]-comb1[,2]
diff2<- comb2[,1]-comb2[,2]
plot(diff1, diff2)
abline(a=0, b=1, h=0.0, v=0.0)
abline(a=0, b=1, h=0.01, v=-0.01, col='grey')

#Flag pairs where direction flips (where strain 1 is more fit in 1 conditions and less fit in the other)
flag.pair<- rep(0, times=length(diff1))
flag.pair[diff1< -0.0 & diff2> 0.01 ]<-1
flag.sts<-cbind( names(comb1$Var1)[flag.pair==1], names(comb1$Var2)[flag.pair==1]  )
flag.sts<-flag.sts[order(flag.sts[,1], flag.sts[,2]),]
#names(comb1$Var1)==names(comb2$Var1) ##check
```
...
Same, but use max.od instead
```{r}
max.od.sub<-d1d[d1d$temp %in% c('33','35','37'),]
mod.st1<-lme(max.od  ~ st +  temp + Diagnosis ,  random = ~ 1|ID,data=max.od.sub[max.od.sub$anaerobic=='1',])
coef.fe.mod.st1<-fixef(mod.st1)
coef.st1.maxod<-coef.fe.mod.st1[substr(names(coef.fe.mod.st1),1,2)=='st' ]

mod.st2<-lme(max.od  ~ st +  temp + Diagnosis ,  random = ~ 1|ID,data=max.od.sub[max.od.sub$anaerobic=='2',])
coef.fe.mod.st2<-fixef(mod.st2)
coef.st2.maxod<-coef.fe.mod.st2[substr(names(coef.fe.mod.st2),1,2)=='st' ]

##
plot.range<-range(c(coef.st1.maxod,coef.st2.maxod))
plot(coef.st2.maxod,coef.st1.maxod, ylim=plot.range, xlim=plot.range)
abline(a=0, b=1)

#all.perm<-expand.grid(coef.st.fixed.od.1,coef.st.fixed.od.2)
comb1<-expand.grid(coef.st1.maxod,coef.st1.maxod)
comb2<-expand.grid(coef.st2.maxod,coef.st2.maxod)
...
plot(diff1, diff2)
abline(a=0, b=1, h=0.0, v=0.0)
abline(a=0, b=1, h=0.02, v=-0.02, col='grey')

#Flag pairs where direction flips (where strain 1 is more fit in 1 conditions and less fit in the other)
flag.pair<- rep(0, times=length(diff1))
flag.pair[diff1< -0.02 & diff2> 0.02 ]<-1
flag.sts<-cbind( names(comb1$Var1)[flag.pair==1], names(comb1$Var2)[flag.pair==1]  )
flag.sts<-flag.sts[order(flag.sts[,1], flag.sts[,2]),]
#names(comb1$Var1)==names(comb2$Var1) ##check
```
...
sub$decline.quant[is.nan(sub$decline.quant)]<-0

for(i in c('max.od',  "max.deriv","max.deriv2.time",'decline.quant' )){
  print(i)
...
## Late stage effects post peak
In some strains, the growth curve levels out, while in others it drops sharply. This also seems to differ for aerobic and anaerobic in some instances. Does the peak to end ratio differ by strain, and does this differ for aerobic vs anaerobic
--Patterns seems to be mostly simialr for the top serotypes in both aerobic and anaerobic. 
--Positive coefficients mean that the serotype experienced a less intense dip
```{r st.oxygen.interact, fig.width=10.6, fig.height=5.5}
sub<-d1d
sub$decline.quant[is.nan(sub$decline.quant)]<-0
col.ana2<-c('#1b9e77','#7570b3')
...
sub$anaerobic<-factor(sub$anaerobic)
sub$Diagnosis<-factor(sub$Diagnosis)
  form1<-as.formula(paste0( 'decline.quant','~st + anaerobic +Diagnosis+ temp' ))
  mod4<-lme(form1,  random = ~ 1|ID,data=sub)
  summary(mod4)
  coefs<-fixef(mod4)
  coefs.st<-sort(coefs[substr(names(coefs),1,2)=='st'])
  coefs.st
```
...
#Quantfy intensity of the decline (average negative derivative)
d1d$decline.quant[is.nan(d1d$decline.quant)]<-0
mod.st2<-lme(decline.quant  ~   anaerobic*temp + Diagnosis*anaerobic ,  random = ~ 1|ID,data=d1d)
strain.effect.ave.decline<-ranef(mod.st2)
strain.effect.ave.decline$ID<-row.names(strain.effect.ave.decline)
strain.chars<-unique(d1d[,c('ID', 'st')])
strain.effect.ave.decline<-merge(strain.effect.ave.decline, strain.chars, by='ID' , all=TRUE)
#export for shreyas
write.csv(strain.effect.ave.decline,'../strain.effect.decline.csv')

```

#Re-run for anaerobic and aerobic
```{r st.effect.od2, fig.width=8, fig.height=4.5}
#Max OD ...note can't have Dx*temp interaction bc it won't converge
mod.st2<-lme(max.od  ~   temp + Diagnosis ,  random = ~ 1|ID,data=d1d[d1d$anaerobic=='1',])
strain.effect.max.od<-ranef(mod.st2)
strain.effect.max.od$ID<-row.names(strain.effect.max.od)
strain.chars<-unique(d1d[,c('ID', 'st')])
strain.effect.max.od<-merge(strain.effect.max.od, strain.chars, by='ID' , all=TRUE)
#export for shreyas
write.csv(strain.effect.max.od,'../strain.effect.max.od.anaerobic.csv')

mod.st2<-lme(max.od  ~   temp + Diagnosis ,  random = ~ 1|ID,data=d1d[d1d$anaerobic=='2',])
strain.effect.max.od<-ranef(mod.st2)
strain.effect.max.od$ID<-row.names(strain.effect.max.od)
strain.chars<-unique(d1d[,c('ID', 'st')])
strain.effect.max.od<-merge(strain.effect.max.od, strain.chars, by='ID' , all=TRUE)
plot(strain.effect.max.od$st, strain.effect.max.od$`(Intercept)`)
abline(h=0)
#export for shreyas
write.csv(strain.effect.max.od,'../strain.effect.max.od.aerobic.csv')
...
repeat for anaerobic and aerobic
```{r}
#Max OD ...note can't have Dx*temp interaction bc it won't converge
mod.st2<-lme(log(od.fixed.time+0.01)  ~   temp + Diagnosis ,  random = ~ 1|ID,data=od.fixed.time[od.fixed.time$anaerobic=='1',])
strain.effect.max.od<-ranef(mod.st2)
strain.effect.max.od$ID<-row.names(strain.effect.max.od)
strain.chars<-unique(d1d[,c('ID', 'st')])
strain.effect.max.od<-merge(strain.effect.max.od, strain.chars, by='ID' , all=TRUE)
write.csv(strain.effect.max.od, '../od.fixed.time.anaerobic.csv')

mod.st2<-lme(log(od.fixed.time+0.01)  ~   temp + Diagnosis ,  random = ~ 1|ID,data=od.fixed.time[od.fixed.time$anaerobic=='2',])
strain.effect.max.od<-ranef(mod.st2)
strain.effect.max.od$ID<-row.names(strain.effect.max.od)
strain.chars<-unique(d1d[,c('ID', 'st')])
strain.effect.max.od<-merge(strain.effect.max.od, strain.chars, by='ID' , all=TRUE)
write.csv(strain.effect.max.od, '../od.fixed.time.aerobic.csv')
```
...
carr1<-cov1[,c('st','SLEEMANCARRN','NORWAYCARR_PRE','MASSCARR01' ,'GREECECARRN' )]
...
#test Gal
ps.reg.results.gal<-lapply(covar.list, reg.func.logistic,ds=ps2, outcome.name='Gal')
aic.gal<-sapply(ps.reg.results.gal, function(x) AIC(x$reg.pc1) )
or.gal<-cbind(t(exp(sapply(ps.reg.results.gal, function(x) coef(x$reg.pc1) ))) ,covar.list)

print(cbind.data.frame(covar.list,aic.uronic))

#do a monte carlo test
 mc1<-ps2[!is.na(ps2$uronic) & !is.na(ps2$coef.st.max.od.1.2),c('NAC','uronic','Gal','coef.st.max.od.1.2')]
 ...
 diff.mean.NAC<- obs.means.NAC[2]- obs.means.NAC[1]
   ...
   spl1.obs.Gal<-split(mc1, mc1$Gal)
 obs.means.Gal<-sapply(spl1.obs.Gal, function(x) mean(x$coef.st.max.od.1.2))
 diff.mean.Gal<- obs.means.Gal[2]- obs.means.Gal[1]
 
 
 diff.mc.uronic<-rep(NA, 999)
 diff.mc.NAC<-rep(NA, 999)
  diff.mc.Gal<-rep(NA, 999)

  for(i in 1:999){
   mc.shuffle<-mc1
   mc.shuffle$shuffle_uronic<-sample(mc.shuffle$uronic)
   mc.shuffle$shuffle_NAC<-sample(mc.shuffle$NAC)
   mc.shuffle$shuffle_Gal<-sample(mc.shuffle$Gal)

      spl1.obs.uronic<-split(mc.shuffle, mc.shuffle$shuffle_uronic)              
   spl1.obs.NAC<-split(mc.shuffle, mc.shuffle$shuffle_NAC)
   spl1.obs.Gal<-split(mc.shuffle, mc.shuffle$shuffle_Gal)

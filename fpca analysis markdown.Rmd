...
```{r epi_cor}
carr.comp2<-carr.comp[carr.comp$anaerobic != '0' & !is.na(carr.comp$ratio_min_max_rate) ,]

#CFR Harboe
  ds<-carr.comp2[,c('ratio_min_max_rate', 'CFRHARBOE','anaerobic','t.max.deriv1', 'temp','id' )]
  ds<-ds[complete.cases(ds),]
  reg.cfr<-lme(sqrt(ratio_min_max_rate) ~ CFRHARBOE + anaerobic +t.max.deriv1 + temp , random=~1|id, data=ds )  
 ...
 #Total carbon
  ...
  ds<-ds[complete.cases(ds),]
  reg.carbon<-lme(sqrt(ratio_min_max_rate) ~ TOTALCARBONREPEAT + anaerobic +t.max.deriv1 + temp , random=~1|id, data=ds )
 ...
 #PS components
   ...
   ds<-ps2[,c('ratio_min_max_rate', 'uronic','NAC','anaerobic','t.max.deriv1', 'temp','id' )]
   ds<-ds[complete.cases(ds),]
   reg.ps<-lme(sqrt(ratio_min_max_rate) ~ uronic +NAC + anaerobic +t.max.deriv1 + temp , random=~1|id, data=ds )
   round(summary(reg.ps)$tTable,3)
   plot(as.factor(ps2$uronic), sqrt(ps2$ratio_min_max_rate))
```
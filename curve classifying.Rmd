...
Use smooth.spline function, which is same thing they do in the grofit package
```{r prep_spline}
...
Only count max deriv if it occurs n first 15 hours 
```{r extractderiv }
...
```{r flatness.int}
  interaction.plot(mod.df$temp, mod.df$anaerobic, mod.df$ratio_min_max_rate, bty='l')
   ...
   ps2.mid<-mod.df[mod.df$temp %in% c('33','35','37'),]
